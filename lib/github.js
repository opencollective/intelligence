...
function getOrgRepos(githubOrg, page = 1, per_page = 100) {
  if (DEBUG) console.log('get repos from', githubOrg);
  return fetch(`https://api.github.com/orgs/${githubOrg}/repos?page=${page}&per_page=${per_page}&access_token=${process.env.GITHUB_TOKEN}`)
    .then(response => {
      if (DEBUG) {
        logGithubHeaders(response);
      }
